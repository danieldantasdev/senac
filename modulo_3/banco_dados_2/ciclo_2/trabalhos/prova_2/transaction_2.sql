CREATE PROCEDURE T2 (@codm int, @codp int, @data VARCHAR(50), @hora time)
AS
...
BEGIN
 ...
 BEGIN TRY
  IF NOT EXISTS (
    SELECT * FROM Consultas c WHERE (codm = @codm OR codp = @codp) AND hora = @hora
    UNION ALL
    SELECT *, GETDATE() FROM Ambulatorio am  WHERE capacidade > 0
    )
...
EXECUTE T2 79, 1030, "2022/08/01", "09:00"
